...
Be direct, specific, and actionable.`;
    ...
    // Ensure at least 1 alert - pick a company and find relevant market news
    if (results.alerts_created === 0 && allCompanies.length > 0) {
      try {
        // Pick a random company to focus on for today's brief
        const targetCompany = allCompanies[Math.floor(Math.random() * allCompanies.length)];

        const companyBriefPrompt = `TODAY'S DATE: ${today}

You are a VC analyst providing a daily market update for ${targetCompany.name}.

COMPANY: ${targetCompany.name}
DESCRIPTION: ${targetCompany.description || 'Not provided'}
WEBSITE: ${targetCompany.website || 'Not provided'}

Search for the MOST RELEVANT news from the past 7 days that affects ${targetCompany.name}'s market, competitors, or industry.

Focus on:
1. Competitor funding, product launches, or strategic moves
2. Industry trends or regulatory changes
3. Market dynamics or customer behavior shifts
4. Technology developments in their space

Provide ONE specific, actionable insight that an investor in ${targetCompany.name} should know. Be concrete about how it impacts this company.`;

        const briefData = await base44.integrations.Core.InvokeLLM({
          prompt: companyBriefPrompt,
          add_context_from_internet: true,
          response_json_schema: {
            type: "object",
            properties: {
              title: { type: "string" },
              summary: { type: "string" },
              impact_on_company: { type: "string" },
              is_opportunity: { type: "boolean" },
              recommended_action: { type: "string" }
            },
            required: ["title", "summary", "impact_on_company"]
          }
        });

        // Build article content with proper null handling
        const summary = briefData.summary || 'Market intelligence update for today.';
        const impactText = briefData.impact_on_company || 'Analysis pending.';
        const actionText = briefData.recommended_action || '';

        let articleContent = summary;
        if (impactText && impactText !== 'Analysis pending.') {
          articleContent += `\n\n**Impact on ${targetCompany.name}:**\n${impactText}`;
        }
        if (actionText) {
          articleContent += `\n\n**Recommended Action:**\n${actionText}`;
        }

        // Create the alert linked to the specific company
        await base44.asServiceRole.entities.Alert.create({
          company_name: targetCompany.name,
          company_stage: targetCompany.stage,
          title: briefData.title || `Market Update: ${targetCompany.name}`,
          description: summary,
          type: briefData.is_opportunity ? 'opportunity' : 'threat',
          category: 'market',
          priority: 'medium',
          impact: impactText,
          source_url: '',
        ...
        console.error('Error creating company brief:', error);
      }
    }
